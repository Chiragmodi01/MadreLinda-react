...
function CartPage() {
  ...
  const applyVoucher = (e) => {
    {
      state.isVoucherApplied
        ? alert("One voucher already used!")
        : voucherCode.some((code) => code === voucher)
        ? dispatch({ type: "APPLY_VOUCHER", payload: 50 })
          ...
          {state.cartItems.length === 0 ? (
            <div className="empty-page-err err-cart">
              ...
              <button
                ...
                className="btn-fat btn-hover cursor-pointer"
              >
                Go to Products Page
              </button>
            </div>
            ...
            state.cartItems.map((product) => {
              return <EachCardCart key={product._id} product={product} />;
            })
          )}
        </main>
        <div className="cartPage_price-details">
          <div className="cartPage_apply-voucher-wrapper">
            <div className="cartPage-apply-voucher-top">
              <input
                type="text"
                className="cartPage_apply-voucher-input"
                placeholder="apply cupon code here"
                disabled={
                  state.cartItems.length === 0 || state.isVoucherDisabled
                }
                value={voucher}
              ...
              <button
                ...
                className="cartPage_apply-voucher-action btn-hover"
                onClick={applyVoucher}
              >
                {state.isVoucherApplied ? "Applied!" : "Apply"}
              </button>
            </div>
              ...
              <ul className="apply-voucher-bottom-list">
                {voucherCode.map((voucher) => {
                  return (
                    <li
                      key={voucher}
                      className="apply-voucher-bottom-list-item"
                      onClick={(e) => setVoucher(e.target.innerText)}
                    >
                      {voucher}
                    </li>
                  );
          ...
          <div className="cartPage_price-details-wrapper">
            <h5 className="price_details-title">
              Price Details ({state.cartItems.length} item)
            </h5>
          </div>
          <div className="price-details_info-wrapper">
            ...
            <div className="price-details_info-each">
              ...
              <p className="price-details_info-digit">
                - ${state.voucherPrice}
              </p>
            </div>
            <div className="price-details_info-each">
              ...
              <p className="price-details_info-digit">
                $
                {state.cartItems.length === 0 ? 0 : 20 * state.cartItems.length}
              </p>
            </div>
