  ...
  return (
    <div className="each-card-wrapper">
      <div className="each-card-top">
        ...
        {(!product.image1 || showImgErrorPlaceholder) && (
          <div className="showErrorDiv">404 Image not found!</div>
        )}
        <LazyLoadImage
          onError={() => setShowImgErrorPlaceholder(true)}
          afterLoad={() => setShowImgPlaceholder(false)}
          effect="opacity"
          src={product.image1}
          alt="product-img"
          className="card-img"
        ...
        <FiHeart
          ...
          className={
            product.fillHeart
              ? "heart-filled cursor-pointer"
              : "each-card_img-overlay icon-heart cursor-pointer"
          }
          onClick={() =>
            dispatch({ type: "ADD_TO_WISHLIST", payload: { product: product } })
          }
        />
      </div>
      <div className="each-card-section-bottom">
        ...
        <div className="card-collection-rating">
          ...
          <p className="card-badge">
            {[...Array(product.rating)].map((el, index) => (
              <BsStarFill
            ...
            {[...Array(5 - product.rating)].map((el, index) => (
              <BsStar fill="var(--secondary-dark)" key={index} size="1em" />
            ))}
          </p>
        </div>
        <button
          className="btn-add-to-bag btn-long btn-hover"
          onClick={() =>
            product.inBag
              ? navigate("../cart", { replace: true })
              : dispatch({
                  type: "ADD_TO_BAG",
                  payload: { product: product, charges: 20 },
                })
          }
        >
